...
async function runH7Check(): Promise<RunResult[]> {
  const h7Results: RunResult[] = [];

  if (runningH7) {
    console.error("SKIPPED: Previous H7 run still in progress");
    return h7Results;
  }
    ...
    for (const entry of symbols) {
      ...
      try {
        ...
        for (let i = 1; i < closedCandles.length; i++) {
          ...
          // Only evaluate 18->1 and 1->8 transitions
...
async function runH7AndNotify(settings: Settings): Promise<void> {
  console.log(`[${new Date().toISOString()}] Running H7 scan...`);
  const h7 = await runH7Check();

  if (h7.length === 0) {
    console.log("H7 scan skipped (no results or H7 disabled)");
    return;
  }
...
async function main(): Promise<void> {
  ...
  if (settings.h7) {
    h7Task = cron.schedule(
      "1 1,8 * * *",
      () => {
        void runH7AndNotify(settings);
      },
      { timezone: settings.timezone }
    );
    console.log("H7 scheduler started with cron 1 1,8 * * * (01:01 and 08:01 ET)");
  }

  const shutdown = (signal: string) => {
    console.log(`Received ${signal}, shutting down gracefully...`);
    baseTask.stop();
    if (h7Task) h7Task.stop();
    process.exit(0);
  };
  ...
  console.log(`Base scheduler started with cron ${settings.cron} (${settings.timezone})`);
}
